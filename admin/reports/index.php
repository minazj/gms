...
<!-- JavaScript code to handle the filtering of goods options and updating tables -->
<script>
    $(function() {
        // Function to fetch and display goods options based on selected date and goods
        function updateGoodsOptions() {
            var selectedDate = $('#date').val();
            var selectedGoods = $('#goods').val();

            // Make an AJAX request to fetch the goods options based on the selected date and goods
            $.ajax({
                url: 'get_goods_options.php',
                type: 'GET',
                data: { date_created: selectedDate, goods: selectedGoods },
                dataType: 'json',
                success: function(response) {
                    // Update the goods options in the select dropdown
                    var goodsSelect = $('#goods');
                    goodsSelect.empty();
                    goodsSelect.append('<option value="">All Goods</option>');
                    $.each(response, function(index, item) {
                        goodsSelect.append('<option value="' + item.name + '">' + item.name + '</option>');
                    });

                    // Perform the filtering based on both selected date and goods
                    filterResults();
                },
                error: function() {
                    console.log('Error fetching goods options.');
                }
            });
        }

        // Function to perform the filtering and update the table results
        function filterResults() {
            var selectedDate = $('#date').val();
            var selectedGoods = $('#goods').val();

            // Make an AJAX request to fetch the filtered results for goods available table
            $.ajax({
                url: 'get_filtered_goods_available.php',
                type: 'GET',
                data: { date_created: selectedDate, goods: selectedGoods },
                dataType: 'html',
                success: function(response) {
                    // Update the table with the filtered results for goods available
                    $('#goods_available_table').html(response);
                },
                error: function() {
                    console.log('Error fetching filtered goods available results.');
                }
            });
        }

        // Call the updateGoodsOptions function on page load
        updateGoodsOptions();

        // Call the updateGoodsOptions function whenever the date input is changed
        $('#date').change(function() {
            updateGoodsOptions();
        });

        // Call the updateGoodsOptions function whenever the goods input is changed
